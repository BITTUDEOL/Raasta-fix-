...
import { storageUtils, validateUser } from './utils/storage';
...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser && validateUser(savedUser)) {
      ...
      storageUtils.setCurrentUser(null); // Clear bad/corrupt user
      setCurrentUser(null);
    ...
    setLoading(false); // <-- UI will not render until initialization is done
  }, []);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-black">
        <span className="text-lg text-gray-500 dark:text-gray-100 animate-pulse">Loading...</span>
      </div>
    );
  }
