...
// Validate a loaded user object for the minimum fields and correct types
export function validateUser(user: any): user is User {
  ...
  if (
    ...
    (user.role !== 'citizen' && user.role !== 'authority' && user.role !== 'admin')
  ) return false;
  if (typeof user.reportsSubmitted !== 'number') return false;
  if (typeof user.reportsResolved !== 'number') return false;
  if (typeof user.reputation !== 'number') return false;
  if (typeof user.joinedAt !== 'string') return false;
  if (!Array.isArray(user.notifications)) return false;
  if (typeof user.rewardsEarned !== 'number') return false;
  return true;
}
